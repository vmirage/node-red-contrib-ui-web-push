...
self.addEventListener('notificationclick', async function (event) {
    ...
    if (event.action) {
        if (event?.notification?.data?.open?.hasOwnProperty(event.action)) {
            url = event.notification.data.open[event.action];
            url = url ? url : event.notification.data.link;
            url = url ? url : event.action;
            if (url.indexOf('://') == -1) {
                url = new URL(url, nodeRedUrl).toString();
            }
        } else if (event?.notification?.data?.get?.hasOwnProperty(event.action)) {
            ...
            let url = event?.notification?.data?.get?.[event.action];
            url = url ? url : event.action;
            if (url.indexOf('://') == -1) {
                url = new URL(url, nodeRedUrl).toString();
            }
    ...
    if (url) {
        // By clicking on the notification, we will open url:
        ...
        // This way we will avoid opening new window/tabsheets over and over again ...
